...
class Trade():
    ...
    def new_trade(self, trade_id: str, order_type: str, side: str,
                  enter_or_exit: str, price: float = 0.0,
        ...
        if self.side == 'long':
            self.side_oppposite = 'short'
        elif self.side == 'short':
            self.side_oppposite = 'long'

    def instrument(self, symbol: str, quantity: int, asset_type: str,
                   sub_asset_type: str = None, order_leg_id: int = 0) -> dict:
    ...
    def modify_side(self, side: Optional[str], order_leg_id: int = 0) -> None:
        ...
        if side:
            ...
            self.order['orderLegCollection']['order_leg_id']['instruction'] = self.order_instructions[self.enter_or_exit][self.side_oppposite]
    ...
    def add_stop_loss(self, stop_size: float, percentage: bool = False):

        if not self._triggered_added:
            self._convert_to_trigger()

        if self.order_type == 'mkt':
            # Point is to call quotes from bitstamp
            price = self.price
